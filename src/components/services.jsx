    ...
    <section className="self-stretch bg-secondary-light-10 overflow-hidden flex flex-row items-center justify-center py-[10px] px-5 box-border max-w-full text-left text-xl text-secondary-dark-20 font-familymain mq750:pt-[42px] mq750:pb-[42px] mq750:box-border mq1050:pt-[65px] mq1050:pb-[65px] mq1050:box-border" data-aos="fade-up">
      <div className="w-[1300px] flex flex-col items-center justify-start gap-[50px] max-w-full mq750:gap-[50px]">
        ...
        <div className="self-stretch flex flex-col items-start justify-start gap-[30px] max-w-full">
          ...
          <div className="self-stretch rounded-xl bg-secondary-light-10 box-border overflow-hidden flex flex-wrap justify-center gap-[29px]  text-30-custom border-[1px] border-solid border-secondary-light-80">

            {/* Left Column */}
            <div className=" rounded-10-custom-border flex-1 flex-col  items-start justify-center p-5 box-border gap-[10px] min-w-[360px]">
              {tabData.map((tab, index) => (
                <div key={index} className={`${activeTab === tab.title ? 'block' : 'hidden'}`}>
                  <div className=" flex flex-col items-start justify-center gap-[20px] ">
                    <h2 className="  text-inherit font-semibold font-inherit  mq450:text-lg mq750:text-24-custom">
                      {tab.title}
                    </h2>
                    ...
                    <Link to="/contact" className="no-underline">
                      <button className="group transition-bg [background:linear-gradient(140.99deg,_#aace88,_#211e3b_76.04%,_#211e3b_84.38%,_#211e3b)] hover:border-primary-80 transition-transform duration-300 ease-in-out transform cursor-pointer group py-[15px] px-7 bg-[transparent] rounded-45.7-custom-border box-border flex flex-row items-center justify-center gap-[10px] min-w-[140px] ">
                        ...
                        <img className="transition-transform duration-300 ease-in-out transform group-hover:text-secondary-light-10 group-hover:translate-x-1 h-6 w-6 relative overflow-hidden shrink-0" alt="" src="/btn-icons.svg" />
                      </button>
                    </Link>
                  </div>
                  <div className="self-stretch flex flex-row flex-wrap items-center justify-start py-2.5 px-0 box-border gap-[10px] min-h-[80px] text-base text-secondary-dark-50 font-custom-bold">
                    <div className="flex flex-row items-center justify-start gap-[4px]">
                      ...
                      <div className="relative leading-[25px] font-medium">Outstanding Return on Investment, unparalleled in the industry</div>
                    </div>
                    <div className="flex flex-row items-center justify-start gap-[4px]">
                      <img className="h-3.5 w-3.5 relative" alt="" src="/c-t-a-buttons.svg" />
                      <div className="relative leading-[25px] font-medium">A dedicated team of developers</div>
                    </div>
                    <div className="flex flex-row items-center justify-start gap-[4px]">
                      <img className="h-3.5 w-3.5 relative" alt="" src="/c-t-a-buttons.svg" />
                      <div className="relative leading-[25px] font-medium">Precise and accurate deliverables</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {/* Right Column */}
            <div className="flex-1 h-[auto] w-fit-content bg-secondary-light-10 box-border overflow-hidden flex flex-wrap items-center justify-center gap-[29px]  text-30-custom min-w-[360px]">
              <img
                src={tabData.find(tab => tab.title === activeTab)?.tabImage}
                alt=""
